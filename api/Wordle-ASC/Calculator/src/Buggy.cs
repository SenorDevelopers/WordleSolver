...
public class Buggy
{
	...
	private async Task CalculateForOneWordAsync(string word)
	{
		var dbContext = new UoW();
		var words = await dbContext.Words!.ToListAsync();
		...
		double entropy = 0;
		var possiblePatterns = GetAllPatternsForAWord(word, wordsText);

		foreach (var pattern in possiblePatterns)
		{
			var wordsNow = PossibleWays(word, pattern, wordsText);
